    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_AddFunc.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: AddFunc.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * AddFunc.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * AddFunc.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * AddFunc (application created by MATLAB Compiler)
        * run_AddFunc.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * AddFunc.dll (shared library, file extension varies by 
          platform)
        * AddFunc.h (library header file)
    ...
    Windows:
        * AddFunc.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * AddFunc.xml (documentation files)
        * AddFunc.pdb (program database file - if DEBUG option 
          selected)
        * AddFunc.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * AddFunc.jar (Java class files and compiled M-code)
    ...
    Windows:
        * AddFunc.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * AddFunc_1_0.dll (COM component)
        * AddFunc.xla (optionally created and placed in
