    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_SubFunc.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: SubFunc.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * SubFunc.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * SubFunc.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * SubFunc (application created by MATLAB Compiler)
        * run_SubFunc.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * SubFunc.dll (shared library, file extension varies by 
          platform)
        * SubFunc.h (library header file)
    ...
    Windows:
        * SubFunc.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * SubFunc.xml (documentation files)
        * SubFunc.pdb (program database file - if DEBUG option 
          selected)
        * SubFunc.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * SubFunc.jar (Java class files and compiled M-code)
    ...
    Windows:
        * SubFunc.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * SubFunc_1_0.dll (COM component)
        * SubFunc.xla (optionally created and placed in
